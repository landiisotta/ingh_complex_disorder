...
# data types to include in the dataset
...
mrns = set(m for m in icd_mrns if len(icd_mrns[m]) >= min_diagn)
oths = set(m for m in all_mrns if len(all_mrns[m]) >= min_diagn)
...
random.shuffle(rndm)
if n_rndm > 0 and len(rndm) > n_rndm:
    rndm = rndm[:n_rndm]
    mrns |= set(rndm)
